...
async def actinit(caller, client, channel):
    ...
    gs = await client.wait_for_message(timeout = 60.0, author = player.user, check = gscheck)
    gschoice = 2
    if not gs is None:
    ...
    if gschoice == 1:
        ...
        classeschoice = await client.wait_for_message(timeout = 60.0, author = player.user, check = classescheck)
        if classeschoice is None:
            ...
            player.class_set(classes)
        else:
            player.class_set(int(classeschoice.content))
        ...
        startevent = await client.wait_for_message(timeout = 60.0, author = player.user, check = starteventcheck)
        starteventchoice = 4
        starteventresult = 3
        ...
        if starteventchoice == 1:
            if starteventresult == 1:
                ...
                player.stats_change(statsname[randstats], 1)
